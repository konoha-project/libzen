...
class BlockNode {
	...
	void InsertUndefinedVariables() {
		...
		int i = 0;
		for(i = 0; i < size; i++) {
			local.put(this.cfg.LocalVars.get(i).NativeName, -1);
		}
		...
		for(i = 0; i < size; i++) {
			if(local.get(this.cfg.LocalVars.get(i).NativeName) == -1) {
...
class ControlFlowGraph { // ControlFlowGraph
	...
	void RenamingVariables() {
		...
		for(i = 0; i < size; i++) {
			stack.put(this.LocalVars.get(i).NativeName, new Stack<ZNode>());
		}
	...
	private ZNode TopStack(ZNode Node, HashMap<String, Stack<ZNode>> stack) {
		if(Node instanceof ZGetNameNode) {
			...
			assert(Node instanceof ZVarNode);
			String n = ((ZVarNode)Node).NativeName;
			return stack.get(n).peek();
		}
	}
